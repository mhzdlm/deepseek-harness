# Agent Note: T4.5 — 跨三层的错误上抛：状态、切片与待办

Status: implemented

[English](2026-08-26-rlm-error-surfacing-layers.md) | 中文
...
- 缺口：**GUI 是否把这些 code 渲染成"QUOTA/auth/network"友好文案**尚未核实（shell/web 渲染层）。这是 UI 依赖项，不在 rlm 包内。
...
- 修复后（`packages/rlm/plugin-rlm-kernel/src/kernels.ts` 的 `flushSnapshot` 与 `index.ts` 的 sweep）：
...
- 缺口：这些字段**是否在某 UI 渲染层被高亮**（而非只在原始事件里）尚未核实——UI 依赖项。
- 建议：渲染层在 `failedJudges`/`failedLabels` 非空时亮出警示块，列出失败的评委/引用标签。

## Decision

- `kernels.ts` 的 `flushSnapshot`：捕获 `snapshotError` 并填入事件 `error` 字段。
- `index.ts`：idle sweep 顶层失败从 `console.warn` → `ctx.logger.warn`，改为结构化记录。
- `session-stats.mjs --snapshots`：摘要新增 `failure reasons`，时间线行内显示 `error`。
- 测试：`snapshot-rotation.spec.ts` 新增/修正断言——reclaim 失败事件携带真实错误文本（原断言 `'snapshot failed'` 占位符已改为具体 message）。
- 验证：合成会话（1 个 ok + 1 个 failing reclaim）经 `--snapshots` 正确显示 `error="dill pickle error: ..."`；kernel 单测 10/10 通过；typecheck 通过。

## Alternatives considered

- **不改 LLM 错误码体系**：该体系已完备，重复造轮子无意义。
- **①/③ 的 GUI 渲染**：属于 shell/web 渲染层，超出 rlm 包与本次"内核诊断上浮"的范围；本轮只记录现状与设计，移交后续 UI 工作。
