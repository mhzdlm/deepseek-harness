# Agent Note: 2026-08-25 — Kernel keep-alive leases (NEXT T3.2 Phase A)
...
- 收益：内核 idle-reclaim/session-dispose 现在支持显式 lease（pin/unpin）与 LRU 上限驱逐——被 lease 的内核永不被上限驱逐；leased 内核在 dispose 前强制 `snapshotState()` 并带重试兜底，避免临时 dill 失败静默丢失 lease 持有的命名空间。
- 代价：per-lease TTL 推迟给 Phase B 消费者；Phase B jobs/goal/schedule 接线与 Phase C supervisor 仍记录/推迟；上游触发未暴露 task-window 事件，故暂无 rlm 侧 lease 消费者。
