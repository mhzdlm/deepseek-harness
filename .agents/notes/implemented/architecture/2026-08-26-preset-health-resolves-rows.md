...
What the walk gives up: a package resolvable only through a loader hook — an import map, or a tree with no `node_modules` — is reported broken. An installed harness meets this: `dsh plugin install` puts every plugin beside the roster. A pnpm dev checkout does not hoist workspace packages to the root `node_modules`, so the walk misses them even though the host resolver still finds them; the hook below covers that case.

**An optional resolver backs the walk for dev layouts.** `AgentPresets` accepts a `resolveModule` Config hook, threaded through `discoverPresets` and consulted only when `packageInstalled` returns false. Production passes none, so the disk walk stays authoritative and unchanged. The rlm verifier suite injects a Vite tsconfig-paths resolver — the same one every other `@deepseek-ai/dsh-*` specifier in the suite uses — so a pnpm checkout that symlinks workspace packages into each consumer's `node_modules` instead of hoisting them to the root still resolves the preset's rows, and the suite goes 40/40. The hook returns a boolean and imports nothing during discovery; it runs only on the miss path, so it never re-introduces the import-side-effect cost this decision rejected.
