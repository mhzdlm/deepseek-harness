...
`overrides` 并入时不复筛；调用方只传内部 `RLM_*` 接线。`platform` 与 `source` 参数使两种大小写语义在任何主机上确定性可测（`tests/kernel-env.spec.ts` 共 10 项，注入式 source，不改真实 `process.env`）。`scripts/audit-vendor.mts` 新增三个 #14 检查项（共 33 项）钉住导入与调用点防回归。
...
- `scripts/audit-vendor.mts` #14 检查：`index.ts` 的共享模块导入、`fork-server.ts` 的净化启动环境、`bootstrap.ts` 的净化引导环境。
