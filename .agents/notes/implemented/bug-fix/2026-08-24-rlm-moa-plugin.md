...
- Config holds named presets (`referenceModels[]` + `aggregator`, per-preset `referenceMaxTokens`, `referenceTimeoutMs`, `degradedPolicy`). With none configured, a built-in preset fans out to two DeepSeek models and aggregates on a stronger one. `dataDir` defaults to the shared `~/.dsh/rlm`; traces append JSONL under `<dataDir>/moa-traces/<sessionId>.jsonl`.
