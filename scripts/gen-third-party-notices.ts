...
/**
 * Runtime dependencies whose license is not on the permissive list but whose
 * distribution the project owner has explicitly reviewed and authorized.
 * Keyed by exact npm package name; the recorded license string must match the
 * installed manifest, so any license change re-arms the gate (fail closed).
 */
const RUNTIME_LICENSE_AUTHORIZATIONS: Record<string, { license: string; reason: string }> = {
  // Jupyter kernel ZMQ transport — the only mature Node binding for the
  // Jupyter wire protocol (control/shell/iopub channels). MPL-2.0 is
  // file-level weak copyleft: shipping the package unmodified as a linked
  // dependency only obliges retaining its license notice, which this
  // document already does for every dependency.
  zeromq: {
    license: 'MIT AND MPL-2.0',
    reason: 'file-level weak copyleft; linked unmodified; license notice retained',
  },
}

function isRuntimeLicenseAuthorized(name: string, license: string): boolean {
  const authorization = RUNTIME_LICENSE_AUTHORIZATIONS[name]
  return authorization !== undefined && authorization.license === license
}
...
/**
 * Render the note documenting owner-authorized runtime licenses — copyleft
 * terms the project ships under an explicit, recorded decision — or nothing
 * when every runtime dependency is either permissive or owner-identity-scoped.
 */
function renderAuthorizedRuntimeNote(deps: ExternalDep[]): string {
  if (deps.length === 0) return ''
  const lines = deps.map((dep) => {
    const authorization = RUNTIME_LICENSE_AUTHORIZATIONS[dep.name]
    return `- \`${dep.name}\` (\`${dep.license}\`): ${authorization?.reason ?? 'authorized for distribution.'}`
  })
...
export function render(): string {
  ...
  const nonPermissiveRuntime = runtimeDeps.filter(dep =>
    ...
    && !isRuntimeLicenseAuthorized(dep.name, dep.license),
  )
  ...
  const authorizedRuntime = runtimeDeps.filter(dep => isRuntimeLicenseAuthorized(dep.name, dep.license))
...
${renderAuthorizedRuntimeNote(authorizedRuntime)}
