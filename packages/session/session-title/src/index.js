...
export class SessionTitleService extends Service {
    ...
    register(provider) {
        ...
        const dispose = this.ctx.effect(function* () {
            ...
            yield async () => {
                ...
                for (const state of this.work.values()) {
                    if (state.pending?.registration === registration)
                        delete state.pending;
    ...
    onMainRequest(options) {
        if (!this.serviceActive() || options.sessionId === undefined || !isAgentLoopRequest(options))
            return;
    ...
    serviceActive() {
        return !this.lifetime.signal.aborted
            ...
            && this.ownerFiber.state === 2 /* FiberState.ACTIVE */;
    }
