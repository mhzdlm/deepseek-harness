...
import { discoverPresets, SHIPPED_PRESET_ROOT, USER_PRESET_DIR } from "./discovery.js";
import { copyComposition, deleteComposition, presetExists, readComposition } from "./authoring.js";
import { livePresetMounts, mountPreset, serviceForAgent, standingMountFor } from "./mount.js";
import { fileComposition, mountedCompositionRows, } from "./composition-inventory.js";
...
export { COMPOSITION_FILE, discoverPresets, scanRoot, SHIPPED_PRESET_ROOT } from "./discovery.js";
export { METADATA_FILE, readPresetMetadata, renderPresetMetadata, } from "./metadata.js";
export { inactiveRows, leakedServices, livePresetMounts, mountPreset, serviceForAgent, standingMountFor, } from "./mount.js";
export { copyComposition, deleteComposition, readComposition, writableRoot } from "./authoring.js";
...
let AgentPresets = (() => {
    ...
    return class AgentPresets extends _classSuper {
        static {
            ...
            __esDecorate(this, null, _remoteExportCopy_decorators, { kind: "method", name: "remoteExportCopy", static: false, private: false, access: { has: obj => "remoteExportCopy" in obj, get: obj => obj.remoteExportCopy }, metadata: _metadata }, null, _instanceExtraInitializers);
        ...
        static Config = z.object({
            ...
            resolveModule: z.any().default(undefined),
        });
        /**
        ...
        constructor(ctx, config) {
            ...
            if (baseUrl === undefined) {
                ...
                throw new Error('agent-presets: the roster needs `ctx.baseUrl` to resolve the plugins a composition names; '
            ...
            ctx.on('agent/created', ({ agent }) => {
                if (this.resolvedRoots.length === 0)
                    return;
                if (this.composedPreset(agent.ctx) !== undefined)
                    return;
                ctx.logger.warn(`agent "${agent.id}" was published without joining an agent preset; `
         ...
         */
        async list() {
            return await discoverPresets(this.resolvedRoots, this.harnessBase, this.config.resolveModule);
        }
        /**
        ...
        async resolve(id) {
            ...
            if (found === undefined) {
                ...
                throw new RemoteError('agent-preset/not-found', `agent-presets: preset "${wanted}" not found (available: ${available.join(', ') || 'none'})`, { agentPreset: wanted, available });
            }
            return found;
        }
        /**
        ...
        async resolveMountable(id) {
            ...
            if (preset.broken !== undefined) {
                throw new RemoteError('agent-preset/invalid', `agent-presets: preset "${preset.id}" failed to mount: ${preset.broken}`, { agentPreset: preset.id, reason: preset.broken });
            }
            return preset;
        }
        /**
        ...
        async remove(id) {
            ...
            if (this.settings?.get().default !== id)
                return;
            await this.settingsService?.mutate(SETTINGS_NAMESPACE, [{ op: 'unset', path: ['default'] }]);
        }
        /**
        ...
        async swap(agent, agentPreset) {
            ...
            if (boundary !== undefined
                ...
                throw new RemoteError('agent-preset/locked', `session "${agent.id}" has already started; its agent preset is fixed`, { sessionId: agent.id, agentPreset });
            }
            ...
            const created = (async () => {
                ...
                try {
                    ...
                    if (stamp === undefined) {
                        ...
                        throw new RemoteError('agent-preset/invalid', `agent-presets: preset "${preset.id}" failed to mount: ${reason}`, { agentPreset: preset.id, reason });
                    }
