...
import { classifyRowSpecifier } from "./specifier.js";
...
async function rowResolves(row, presetBase, harnessBase, moduleResolver) {
    if (row.kind === 'builtin')
        return true;
    if (row.kind === 'package') {
        if (isBuiltin(row.specifier))
            return true;
        if (packageInstalled(row.specifier, harnessBase))
            return true;
        ...
        if (moduleResolver)
            return !!(await moduleResolver(row.specifier, harnessBase));
        return false;
    }
 ...
 */
async function unresolvableRows(rows, presetBase, harnessBase, at = '', moduleResolver) {
    const found = [];
    for (const [index, entry] of rows.entries()) {
        ...
        if (row.group === true) {
            found.push(...await unresolvableRows(row.config, presetBase, harnessBase, positional, moduleResolver));
            continue;
        }
        if (await rowResolves(classifyRowSpecifier(row.name), presetBase, harnessBase, moduleResolver))
            continue;
 ...
 */
async function compositionProblem(path, harnessBase, moduleResolver) {
    let content;
    try {
    ...
    const unresolvable = await unresolvableRows(rows, presetBase, harnessBase, '', moduleResolver);
 ...
 */
export async function scanRoot(root, harnessBase, moduleResolver) {
    ...
    for (const child of children) {
        if (!child.isDirectory() || !PRESET_ID.test(child.name))
            continue;
 ...
 */
export async function discoverPresets(roots, harnessBase, moduleResolver) {
    const byId = new Map();
