 ...
 */
import { existsSync, mkdirSync, readdirSync, readFileSync, renameSync, rmSync, statSync, watch as fsWatch, writeFileSync } from 'node:fs';
import path from 'node:path';
import { parseNote, publishedDir } from "./storage.js";
/** The mailbox scope (single, family-wide). */
export const MAILBOX_SCOPE = { kind: 'mailbox' };
 ...
 */
export async function publishToMailbox(store, gate, inputs) {
    const mailbox = gate.mailboxScope ?? MAILBOX_SCOPE;
    const report = { published: 0, observed: 0, subjects: [], conflicts: [], frozenSkips: [] };
    ...
    for (const input of inputs) {
        ...
        report.published += 1;
        if (!input.revision && existing.length > 0)
            report.conflicts.push(input.subject);
        if (!report.subjects.includes(input.subject))
            report.subjects.push(input.subject);
    }
 ...
 */
export async function syncMailboxProjection(store, memoryDir) {
    ...
    const titled = (b) => b.title !== undefined && b.subject !== undefined;
    ...
    for (const name of readdirSync(dir)) {
        if (!name.endsWith('.md') || wantFiles.has(name))
            continue;
        const full = path.join(dir, name);
        const existing = parseNote(full);
        if (typeof existing?.frontmatter.subject !== 'string' || existing.frontmatter.subject === '')
            continue;
...
function writeFileSyncIfChanged(file, content) {
 ...
 */
export async function importLegacyNotes(store, memoryDir) {
    await store.loadOnce(MAILBOX_SCOPE);
    const dir = publishedDir(memoryDir);
    if (!existsSync(dir))
        return 0;
    ...
    for (const name of readdirSync(dir)) {
        ...
        const subject = typeof note.frontmatter.subject === 'string' && note.frontmatter.subject !== ''
        ...
        if (active && active.content === note.body)
            continue;
 ...
 */
export async function detectHumanRevisions(store, memoryDir) {
    await store.loadOnce(MAILBOX_SCOPE);
    const dir = publishedDir(memoryDir);
    if (!existsSync(dir))
        return 0;
    ...
    for (const name of readdirSync(dir)) {
        ...
        if (latest && latest.content === note.body)
            continue;
        await store.append(MAILBOX_SCOPE, 'rlm/human-revision', {
            action: 'upsert',
            subject,
    ...
    for (const belief of store.beliefs(MAILBOX_SCOPE)) {
        if (belief.title === undefined || belief.subject === undefined)
            continue;
 ...
 */
export function watchMailboxProjection(store, memoryDir) {
    const dir = publishedDir(memoryDir);
    if (!existsSync(dir))
        return null;
    let timer = null;
 ...
 */
export async function pickupMailboxSeeds(store, sessionScope) {
    await store.loadOnce(MAILBOX_SCOPE);
    const titled = (b) => b.title !== undefined && b.subject !== undefined;
    ...
    const report = { picked: 0, conflicts: [], byId: new Map() };
    for (const [subject, group] of bySubject) {
        const conflict = group.length > 1;
        if (conflict)
            report.conflicts.push(subject);
 ...
 */
export async function proposeCriterion(store, sessionScope, proposal) {
    await store.loadOnce(MAILBOX_SCOPE);
 ...
 * @returns void
 */
export async function approveCriterion(store, proposal) {
    await store.append(MAILBOX_SCOPE, 'rlm/human-revision', {
