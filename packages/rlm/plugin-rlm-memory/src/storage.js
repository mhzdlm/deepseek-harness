 ...
 */
export const SUBDIRS = ['published', 'drafts', 'dialog', 'index', 'logs', 'snapshots', 'archived'];
/**
 * Create the memory directory tree if absent. Idempotent.
 * @param memoryDir - resolved memory root.
 */
export function ensureMemoryDirs(memoryDir) {
 ...
 */
export function dialogPath(memoryDir, sessionId) {
 ...
 */
export function writeDialog(memoryDir, sessionId, jsonl) {
 ...
 */
export function readDialog(memoryDir, sessionId) {
...
function draftPath(memoryDir, kind, sessionId, title) {
 ...
 */
export function writeDraft(memoryDir, note, sessionId, title) {
 ...
 */
export function listDrafts(memoryDir) {
 ...
 */
export function deleteDraft(memoryDir, path) {
 ...
 */
export function publishedDir(memoryDir) {
 ...
 */
function publishedBaseFor(note) {
 ...
 */
export function publishedRelFor(note) {
 ...
 * @returns the absolute path written.
 */
export function writePublished(memoryDir, note, targetRel) {
 ...
 */
export function listPublished(memoryDir) {
 ...
 */
export function writeEmbedding(memoryDir, relPath, vector) {
 ...
 */
export function readEmbedding(memoryDir, relPath) {
 ...
 * @param relPath - the published note relPath.
 */
export function deleteEmbedding(memoryDir, relPath) {
    const p = join(embeddingCacheDir(memoryDir), `${embeddingKey(relPath)}.json`);
    if (existsSync(p))
        rmSync(p, { force: true });
}
 ...
 */
export function readNote(memoryDir, relPath) {
 ...
 */
export function updateUsage(memoryDir, relPath, nowIso) {
    const path = relPath.startsWith(memoryDir) ? relPath : join(memoryDir, relPath);
    ...
    let priorMtime;
    try {
        priorMtime = statSync(path).mtimeMs;
    }
    catch { /* missing stat is harmless */ }
    const updated = {
        frontmatter: {
            ...note.frontmatter,
    ...
    if (priorMtime !== undefined) {
        try {
            utimesSync(path, priorMtime / 1000, priorMtime / 1000);
        }
        catch { /* best-effort */ }
    }
}
 ...
 */
export function snapshotsDir(memoryDir) {
 ...
 */
export function takeSnapshot(memoryDir, relPath, content) {
    ...
    if (existsSync(snapPath)) {
        let n = 2;
        while (existsSync(join(container, `${iso}-${n}.md`)))
            n += 1;
 ...
 */
export function listSnapshots(memoryDir, noteId) {
 ...
 */
export function restoreSnapshot(memoryDir, noteId, snapshotFile) {
 ...
 */
export function archivedDir(memoryDir) {
 ...
 */
export function archiveNote(memoryDir, relPath) {
    ...
    if (!existsSync(src))
        throw new Error(`archiveNote: ${relPath} does not exist under published/`);
    const note = parseNote(src);
    if (!note)
        throw new Error(`archiveNote: ${relPath} is not a valid note file`);
 ...
 * @returns the absolute published note path written.
 */
export function unarchiveNote(memoryDir, archivedRelPath) {
    const norm = archivedRelPath.split(/[\\/]/).filter(Boolean).join('/');
    const src = norm.startsWith(memoryDir) ? norm : join(memoryDir, norm);
    if (!existsSync(src))
        throw new Error(`unarchiveNote: ${norm} does not exist under archived/`);
    const note = parseNote(src);
    if (!note)
        throw new Error(`unarchiveNote: ${norm} is not a valid note file`);
    ...
    const restored = { frontmatter: rest, body: note.body };
 ...
 */
export function listArchived(memoryDir) {
 ...
 */
export function resolvePublishedAbs(memoryDir, noteId) {
 ...
 */
export function toPublishedRel(memoryDir, absPath) {
