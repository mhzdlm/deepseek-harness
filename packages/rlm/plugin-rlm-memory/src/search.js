 ...
 */
import { listPublished, parseNote, readNote, readEmbedding } from "./storage.js";
 ...
 */
export function buildIndex(memoryDir) {
    const notes = [];
 ...
 */
export function search(memoryDir, query, limit, kind) {
    ...
    for (const note of notes) {
        if (kind !== undefined && note.note.frontmatter.kind !== kind)
            continue;
        let score = 0;
 ...
 * @returns ranked hits.
 */
export async function hybridSearch(memoryDir, query, limit, kind, embeddingService) {
    const { notes, inverted } = buildIndex(memoryDir);
    if (notes.length === 0)
        return [];
    const queryTerms = tokenize(query);
