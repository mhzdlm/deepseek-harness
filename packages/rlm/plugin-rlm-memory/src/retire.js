...
import { parseNote, archiveNote, unarchiveNote, listArchived, resolvePublishedAbs, toPublishedRel, } from "./storage.js";
import { withLock } from "./consolidate.js";
/**
...
function daysSince(iso, now = Date.now()) {
    ...
    if (Number.isNaN(then))
        return Number.POSITIVE_INFINITY; // unparseable timestamps are maximally stale
 ...
 */
export function isRetireCandidate(note, options, now = Date.now()) {
 ...
 */
export async function retireNote(memoryDir, noteId, options, force = false) {
    ...
    return withLock(relPath, async () => {
        ...
        // enforce
        if (!existsSync(abs))
            return `No published note found for ${relPath}.`;
        const note = parseNote(abs);
        if (!note)
            return `${relPath} is not a valid note file.`;
 ...
 * @returns a human-readable outcome string for the command layer.
 */
export async function unretireNote(memoryDir, noteId) {
    ...
    return withLock(publishedRel, async () => {
        ...
        if (!found)
            return `No archived note found for ${publishedRel}.`;
 ...
 */
export function listArchivedNotes(memoryDir) {
 ...
 */
function toArchivedRel(relPath) {
