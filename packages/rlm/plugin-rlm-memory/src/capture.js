 ...
 */
import { sanitizeTurns, renderDialogJsonl } from "./sanitize.js";
import { writeDialog, readDialog, writeDraft } from "./storage.js";
import { admitByEvidence } from "./evidence.js";
/**
 ...
 */
function draftFrontmatter(sessionId, source, nowIso, title) {
    return {
 ...
 */
export function parseExtractionProposal(proposalText, sessionId) {
    ...
    for (const item of list) {
        if (typeof item !== 'object' || item === null)
            continue;
        const { title, source, body, kind } = item;
 ...
 */
export function persistCapture(memoryDir, entry, proposals) {
 ...
 */
export async function extractDrafts(subagents, parent, sessionId, dialogText, signal, timeoutMs) {
    if (dialogText.trim().length === 0)
        return [];
    const prompt = [
        { type: 'text', text: extractionPrompt(sessionId, dialogText) },
    ];
...
function extractionPrompt(sessionId, dialogText) {
    return [
