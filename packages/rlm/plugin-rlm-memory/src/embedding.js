 ...
 */
export function createExternalEmbeddingProvider(opts) {
            ...
            for (let i = 0; i < texts.length; i += batchSize) {
                ...
                for (const item of json.data ?? []) {
                    ...
                    if (item.embedding.length !== dim) {
                        throw new Error(`[plugin-rlm-memory] embeddings dimension mismatch: provider returned ${item.embedding.length}, expected ${dim} (embeddingsDim/model mismatch)`);
                    }
                }
            }
            for (let i = 0; i < out.length; i++)
                if (out[i] === undefined)
                    out[i] = new Array(dim).fill(0);
            return out;
        },
    };
}
 ...
 */
export function createFakeEmbeddingService(dim = 16) {
    const vec = (text) => {
