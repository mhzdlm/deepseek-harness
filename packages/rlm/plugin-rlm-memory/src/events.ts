 ...
 * pins that pairing.
 */
export const MEMORY_EVENT_TYPES = ['session/memory-captured'] as const
 ...
 */
export function emitMemoryCapturedEvent(
  session: Pick<Session, 'append'> | null | undefined,
  payload: MemoryCapturedEventData,
): void {
  if (!session) return
  try {
    session.append('session/memory-captured', payload as never)
  } catch {
