...
import { listDrafts, parseNote, deleteDraft, readNote, listPublished, listArchived, toPublishedRel } from "./storage.js";
import { consolidate, rollbackNote } from "./consolidate.js";
import { retireNote, unretireNote } from "./retire.js";
/**
 ...
 */
export function listMemoryText(memoryDir) {
    const paths = listDrafts(memoryDir);
    if (paths.length === 0)
        return '(no memory drafts)';
    const lines = [];
 ...
 */
export function showMemoryText(memoryDir, name) {
    let path;
    try {
 ...
 */
export function deleteMemoryText(memoryDir, name) {
    try {
...
function resolveDraftPath(memoryDir, name) {
    ...
    if (!candidate)
        throw new Error(`no draft note named "${name}"`);
    return candidate;
}
/**
 ...
 */
export async function consolidateText(memoryDir, options) {
 ...
 */
export async function rollbackText(memoryDir, noteId, force) {
 ...
 * memory root. */
function resolvePublishedRel(memoryDir, name) {
    if (name.includes('..'))
        throw new Error(`invalid noteId "${name}": ".." segments are not allowed`);
    if (isAbsolute(name))
        throw new Error(`invalid noteId "${name}": absolute paths are not allowed`);
    let rel;
...
function resolvePublishedId(memoryDir, name) {
 ...
 */
export async function retireText(memoryDir, noteId, force, options) {
 ...
 */
export function archivedText(memoryDir) {
 ...
 */
export async function unretireText(memoryDir, noteId) {
 ...
 */
function resolveArchivedId(memoryDir, name) {
    if (name.includes('..'))
        throw new Error(`invalid noteId "${name}": ".." segments are not allowed`);
    let archivedRel;
    if (isAbsolute(name)) {
        ...
        if (!norm.startsWith(rootNorm + '/'))
            throw new Error(`invalid noteId "${name}": must resolve under ${memoryDir}`);
        ...
        if (match === undefined)
            throw new Error(`unretire: no archived note matches "${name}" (run /memory archived to list ids)`);
        archivedRel = match;
    }
    if (archivedRel.startsWith('published/'))
        archivedRel = `archived/${archivedRel.slice('published/'.length)}`;
    if (!archivedRel.startsWith('archived/'))
        archivedRel = `archived/${archivedRel}`;
...
export { readNote };
//# sourceMappingURL=memory-cmd.js.map