...
async function applyProposals(
	...
	const touched = new Map<string, HarnessEntry>()

	for (const proposal of proposals) {
		const entries = (state.entries[proposal.kind] ??= {})
		const existing = proposal.id ? entries[proposal.id] : undefined
		// Snapshot the target entry (or the tombstone when deleting) once per entry id.
		const entryKey = `${proposal.kind}:${existing?.id ?? ''}`
		if (!touched.has(entryKey)) {
			touched.set(entryKey, existing ?? { kind: proposal.kind } as HarnessEntry)
			snapshot[entryKey] = existing
				? { ...existing }
				: null
		}

		if (proposal.action === 'delete' && existing) {
			delete entries[proposal.id!]
			changes.push(`delete ${proposal.kind}:${proposal.title}`)
		} else if (proposal.action === 'upsert') {
			const now = nowIso()
