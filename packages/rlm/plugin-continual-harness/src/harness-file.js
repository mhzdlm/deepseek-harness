 ...
 */
export function harnessStatePath(baseDir, sessionId) {
 ...
 */
export function globalHarnessStatePath(baseDir) {
 ...
 */
export async function readHarnessStateDetailed(filePath) {
    let mtimeMs = null;
    try {
 ...
 */
export async function readHarnessState(filePath) {
 ...
 */
export async function writeHarnessState(filePath, state, expectedMtimeMs) {
    ...
    if (expectedMtimeMs !== undefined) {
        try {
            ...
            if (error instanceof HarnessConflictError)
                throw error;
 ...
 */
export async function readHarnessStatesDetailed(baseDir, sessionId) {
 ...
 */
export async function writeHarnessStates(baseDir, sessionId, global, local, expected) {
...
export const HARNESS_KINDS = ['prompt', 'memory', 'skill', 'subagent'];
/**
 ...
 */
export function mergeHarnessStates(global, local) {
    const entries = {};
    for (const kind of HARNESS_KINDS) {
        const globalEntries = global.entries[kind];
        const localEntries = local.entries[kind];
        if (globalEntries && Object.keys(globalEntries).length > 0)
            entries[kind] = { ...globalEntries };
 ...
 */
export function splitHarnessStateByScope(merged, globalRefinements) {
    const globalEntries = {};
    const localEntries = {};
    for (const kind of HARNESS_KINDS) {
        ...
        for (const [id, entry] of Object.entries(entries)) {
            if (entry.scope === 'global')
                (globalEntries[kind] ??= {})[id] = entry;
            else
                (localEntries[kind] ??= {})[id] = entry;
        }
    }
    return {
 ...
 * @returns the parsed {@link HarnessStateFile}, or an empty state if missing/corrupt.
 */
export function readHarnessStateSync(filePath) {
    try {
...
function pruneCorruptBackupsSync(filePath) {
    try {
        const dir = path.dirname(filePath);
        const base = path.basename(filePath);
...
function backupCorruptSync(filePath) {
    try {
...
function emptyHarnessState() {
    return { schema: 1, entries: {}, refinements: [] };
}
function isRecord(value) {
    return typeof value === 'object' && value !== null && !Array.isArray(value);
}
//# sourceMappingURL=harness-file.js.map