 ...
 */
import { harnessStatePath, writeHarnessState } from "./harness-file.js";
 ...
 */
export function renderSessionProjection(view) {
    const memories = {};
    for (const action of view.actions) {
        if (!isRecord(action.payload) || action.payload['action'] !== 'loop-begin')
            continue;
 ...
 */
export function registerStoreProjection(store, baseDir) {
    return store.onChange((scope, view) => {
        ...
        void writeHarnessState(target, renderSessionProjection(view)).catch((error) => {
