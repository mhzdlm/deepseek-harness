 ...
 */
export function latestUserQuery(session, maxQueryChars = 200) {
 ...
 */
export function renderRecallSection(query, hits, budgetChars) {
    if (hits.length === 0)
        return '';
    const lines = [`## Relevant Memories (recall of "${query.slice(0, 80)}")`];
    let used = (lines[0] ?? '').length + 1;
    for (const hit of hits) {
        const header = `- **${hit.title}** (${hit.kind}, score ${hit.score.toFixed(2)})`;
        const remaining = budgetChars - used;
        if (remaining <= 0 || header.length > remaining)
            break;
...
//# sourceMappingURL=recall-inject.js.map