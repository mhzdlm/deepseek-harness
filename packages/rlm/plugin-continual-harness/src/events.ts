 ...
 */
export const CONTINUAL_HARNESS_EVENT_TYPES = ['session/memory-recall-inject'] as const
 ...
 */
export function emitRecallInjectEvent(
  session: Pick<Session, 'append'> | null | undefined,
  payload: MemoryRecallInjectEventData,
): void {
  if (!session) return
  try {
    session.append('session/memory-recall-inject', payload as never)
  } catch {
